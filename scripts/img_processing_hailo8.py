...
def letterbox(img : str | cv2.typing.MatLike, new_shape : tuple[int, int]=(640, 640), color=(114, 114, 114)) -> tuple[cv2.typing.MatLike, float, float]:
    ...
    Returns: a new image object (cv2.Mat)"""
...
def draw_boxes(result : np.ndarray[np.float64], img : cv2.typing.MatLike, threshold : float, dw=0, dh=0, r=1, model_input=(640,640)) -> cv2.typing.MatLike:
    """Copy image and draws bounding boxes on image.
    ...
    Returns: new image object with bounding boxes of each detection along with label, confidence and its color."""
    # TODO: change this to draw boxes on original resolution
    ...
    for i, animal_arr in enumerate(result): # loop through each animal class array. i is the animal class
        for detection in animal_arr: # detection = [ymin, xmin, ymax, xmax, confidence]
            if detection[4] >= threshold:
                ymin = int(detection[0] * model_input[1])
                xmin = int(detection[1] * model_input[0])
                ymax = int(detection[2] * model_input[1])
                xmax = int(detection[3] * model_input[0])
                ...
                cv2.rectangle(_img, (xmin, label_ymin-labelSize[1]-10), (xmin+labelSize[0], label_ymin+baseLine-10), color, cv2.FILLED) # Draw box to put label text in
                cv2.putText(_img, label, (xmin, label_ymin-7), cv2.FONT_HERSHEY_SIMPLEX, text_scale, black, text_thickness) # Draw label text
                cv2.rectangle(_img, (xmin, ymin), (xmax, ymax), color, thickness=2) # bounding box
